...
import { createTask, deleteTask as removeTask, getTasks, updateTask } from './api/tasks'
...
function App() {
  ...
  useEffect(() => {
    getTasks()
      .then(setTasks)
      .catch(() => setError('Could not load your tasks. Is the backend running?'))
  ...
  async function addTask(event: FormEvent<HTMLFormElement>) {
    ...
    setError('')
    try {
      const task = await createTask({ title, category: 'Personal', due: 'Today' })
  ...
  async function toggleTask(task: Task) {
    ...
    try {
      ...
      setTasks((current) => current.map((currentTask) => currentTask._id === task._id ? updatedTask : currentTask))
    } catch {
      ...
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col px-5 py-6 sm:px-8 lg:px-12">
        ...
        <section className="grid flex-1 gap-12 py-12 lg:grid-cols-[1fr_2fr] lg:gap-24 lg:py-20">
          ...
          <div className="w-full max-w-2xl">
            <form onSubmit={addTask} className="mb-8 flex items-center gap-3 rounded-2xl border border-black/10 bg-white p-2 pl-5 shadow-[0_12px_35px_rgba(30,48,35,0.06)] dark:border-white/10 dark:bg-white/6 dark:shadow-none"><span className="text-xl text-[#8ca241]">+</span><input value={newTask} onChange={(event) => setNewTask(event.target.value)} placeholder="What needs your attention?" className="min-w-0 flex-1 bg-transparent py-3 text-sm outline-none placeholder:text-black/35 dark:placeholder:text-white/35" /><button type="submit" disabled={isLoading} className="rounded-xl bg-[#213c2d] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#31583f] disabled:cursor-not-allowed disabled:opacity-50">Add task</button></form>

            {error && <p role="alert" className="mb-5 rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700 dark:bg-red-950/40 dark:text-red-300">{error}</p>}
            ...
            <div className="divide-y divide-black/8 dark:divide-white/10">{isLoading ? <p className="py-12 text-center text-sm text-black/45 dark:text-white/45">Loading tasks...</p> : visibleTasks.map((task) => <article key={task._id} className="group flex items-center gap-4 py-5 first:pt-3"><button onClick={() => toggleTask(task)} aria-label={task.completed ? `Mark ${task.title} active` : `Complete ${task.title}`} className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 text-xs transition ${task.completed ? 'border-[#78922c] bg-[#78922c] text-white' : 'border-black/20 text-transparent hover:border-[#78922c] dark:border-white/25'}`}>✓</button><div className="min-w-0 flex-1"><p className={`truncate text-[15px] font-semibold ${task.completed ? 'text-black/35 line-through dark:text-white/35' : ''}`}>{task.title}</p><div className="mt-2 flex items-center gap-2 text-xs text-black/40 dark:text-white/40"><span className={`rounded px-2 py-1 font-semibold ${categoryStyles[task.category] ?? categoryStyles.Personal}`}>{task.category}</span><span>•</span><span>{task.due}</span></div></div><button onClick={() => deleteTask(task._id)} aria-label={`Delete ${task.title}`} title="Delete task" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-xl text-black/30 transition hover:bg-red-50 hover:text-red-500 dark:text-white/30 dark:hover:bg-red-950/40">×</button></article>)}{!isLoading && visibleTasks.length === 0 && <p className="py-12 text-center text-sm text-black/45 dark:text-white/45">Nothing here yet. Enjoy the clear space.</p>}</div>
